...
& $py -m py_compile `
  ...
  ".\balloondb_core\bql_executor.py" `
  ".\balloondb_core\cli.py" `
  ".\balloondb_core\selftest\run_selftest_v03g2.py"

if ($LASTEXITCODE -ne 0) { throw "V03G2_PY_COMPILE_FAILED" }

& $py -m balloondb_core.cli selftest-v03g2 --memory-root $MemoryRoot

if ($LASTEXITCODE -ne 0) { throw "V03G2_SELFTEST_FAILED" }

Write-Host "PASS_RUN_BQL_SELFTEST_V03G2" -ForegroundColor Green
